...
with spends as (
  select extract(year from transaction_date) as year 
    ...
    , lead(spend, 1) over(partition by product_id order by transaction_date desc) as prev_year_spend
  from user_transactions
  order by product_id asc, extract(year from transaction_date) asc
)
select s.year
  , s.product_id
  , s.curr_year_spend
  , s.prev_year_spend
  , Round(((s.curr_year_spend - s.prev_year_spend)/ s.prev_year_spend)*100, 2) as yoy_rate
from spends s;
