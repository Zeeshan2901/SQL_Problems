...
prime_items as (
  select item_type
    , FLOOR(500000/batch_size) * batch_size as prime_occupancy
    from prime_batch
  ...
  select item_type
    , case 
      when item_type = 'prime_eligible' 
        then FLOOR(500000/batch_size) * items_in_batch
      when item_type = 'not_prime' 
        then FLOOR((500000 - (select prime_occupancy from prime_items))/batch_size)* items_in_batch
      end as item_count
  from prime_batch
